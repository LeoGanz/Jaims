...
public class AudioWaveformCreator {
	...
	public BufferedImage createAudioInputStream() throws Exception {
		...
		if (file != null && file.isFile()) {
			try {
				...
				return samplingGraph.createWaveForm(IOUtils.toByteArray(new FileInputStream(file)));
			} catch (Exception ex) {
	...
	class SamplingGraph implements Runnable {
		...
		public BufferedImage createWaveForm(byte[] audioBytes) {
			...
			int w = (int) (getAudioFileLengthInSeconds() * 10);
			int h = 90;
			int[] audioData = null;
		...
		public BufferedImage saveToFile(String filename) {
		...
		private double getAudioFileLengthInSeconds() {
			...
			try {
				...
				durationInSeconds = (frames + 0.0) / newFormat.getFrameRate();
	...
	private static void printUsage() {
		...
		System.out.println(
				"AudioWaveformCreator usage: java AudioWaveformCreator.class [path to audio file for generating the image] [path to save waveform image to]");
	}
}
