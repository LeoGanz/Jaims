...
public class DFEWindow extends ParentPanel {
	...
	private JPanel buildTopPanel() {
		...
		pTop.setLayout(new BoxLayout(pTop, BoxLayout.LINE_AXIS));
		{
			...
			JLabel titel = new JLabel("Direct File Exchange with " + nickname);
			titel.setFont(new Font("SansSerig", Font.PLAIN, 18));
	...
	public void initialiseConnection() {
		...
		SendableDFEInitiation sendable = new SendableDFEInitiation(guiMain.getSettings().getDFEPort());
		sendable.setRecipient(userID);
		guiMain.sendSendable(sendable);
	}
	...
	public void addNewObject(SDFEFileInfo info) {
		...
		DFEObject dfeo = new DFEObject(info.getFileID(), info.getSender(), info.getRecipient(), info.getFileName(), "",
				info.getExtension(), info.getFileType(), info.getDateSent());
