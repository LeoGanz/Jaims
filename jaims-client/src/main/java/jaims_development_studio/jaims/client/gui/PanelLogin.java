...
public class PanelLogin extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5372558141530441259L;
	...
	public PanelLogin(JFrame caller, MainFrame mf) {
		setLayout(new BorderLayout());
		initGUI(caller, mf);
	}
	
	private void initGUI(JFrame caller, MainFrame mf) {
		try {
			...
			lblRegistrationLink.addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent arg0) {
					if (registrationExists == false) {
						...
						new RegistrationWindow(caller, mf.getConnection());
					} else {
	...
	private void sendLogin(MainFrame mf) {
		mf.getConnection().sendSendable();
	}
