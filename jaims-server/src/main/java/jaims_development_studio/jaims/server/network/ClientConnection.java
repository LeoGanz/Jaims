...
public class ClientConnection implements Runnable {
		...
		try {
			...
			while (true)
				try {
					...
					if (user == null)
						if (!((sendable.getType() == SendableType.LOGIN) || (sendable.getType() == SendableType.REGISTRATION))) {
	...
	private void manageSendSendable(Sendable sendable) {
		if (sendable.getType() == SendableType.MESSAGE)
			((SendableMessage) sendable).setTimestampServerSent();
		else if (sendable.getType() == SendableType.MESSAGE_RESPONSE) {
